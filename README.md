...
* **Motor Drivers:** Custom power distribution circuit using SI2302 N-Channel MOSFETs (SOT-23), SOT23-to-DIP routing adapters, and 1N4148 flyback diodes. **CRITICAL:** Each MOSFET gate is grounded with a 1kΩ - 100kΩ pull-down resistor to prevent floating-gate startup spin-outs and to safely bypass the GPIO 12 strapping pin boot-loop.
...
## 🔌 Hardware Pinout Configuration (ESP32-CAM)
The default pins have been explicitly remapped to the MicroSD card bus to preserve the PSRAM and native camera lanes. This ensures a camera module can be utilized in future iterations without hardware conflicts (Note: The MicroSD slot is permanently disabled in this configuration).

* **IMU SDA:** GPIO 14
* **IMU SCL:** GPIO 13
* **Front-Left Motor:** GPIO 12 (Requires pull-down resistor to boot safely)
* **Front-Right Motor:** GPIO 15
* **Rear-Left Motor:** GPIO 2
* **Rear-Right Motor:** GPIO 4 (Shares the onboard Flash LED circuit; LED will pulse with PWM)
...
* **Throttle (Altitude):** Controlled via Z-axis acceleration "bumps". An upward bump adds a fixed step of base power (drone climbs). A downward bump subtracts the exact fixed step of base power (returns to hover, or descends if bumped again).
